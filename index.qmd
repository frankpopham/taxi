...
First, we store the data in an efficient format using [Paraquet](https://parquet.apache.org/docs/).

Second, we analyse Paraquet files using [Apache Arrow in R](https://arrow.apache.org/docs/r/) and [DuckDB](https://duckdb.org/) for SQL.
...
Paraquet format is specifically designed for working with big data efficiently. The January 2014 data in Paraquet format is just 0.164GB. In Paraquet format all 24 months occupy 4.2GB, which is much more manageable . As an R user this is how I am going to illustrate the functionality via R but DuckDB can be used without R of course.
...
The question we are going to ask is how many journeys started in each of New York's boroughs during the two years. We need to join the names of the boroughs to the main data so we set up the CSV file as a Paraquet format.
...
It is possible to efficiently store and manipulate big data from R. Arrow commands are linked to dplyr and so while there is a lot you can do with them, the aim will often to be to work to reduce big data to a smaller size to work directly with in R memory (RAM). The speed of DuckDB and Arrow are amazing.
